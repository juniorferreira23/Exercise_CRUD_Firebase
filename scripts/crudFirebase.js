...
import { getFirestore, collection, doc, getDocs, getDoc, addDoc, updateDoc, deleteDoc, onSnapshot } from "https://www.gstatic.com/firebasejs/9.6.8/firebase-firestore.js"
...
export const firebase = {
    ...
    getUserId: function(event){ //Ao executar vai até o TR da tabela e pegar o id
        let user = event.target.parentElement.parentElement;
        this.userId = user.id
    },


    deleteUserDB: async function(event){//10 - deletando dados
    
        this.getUserId(event)
        deleteDoc(doc(db, collectionName, await this.getDocId()));
    ...
    pasteEditValue: async function(name, lastname, gender, age, cpf){ //11 - quando abrir o form de editar dados ele já trás os campos preenchidos com o do campo clicado
        ...
        let docRef = await doc(db, collectionName , await this.getDocId())
    ...
    updateUserDB: async function(name, lastname, gender, age, cpf){ //12 - pegar os dados do form edit e envia para o banco de dados
        ...
        let ref = await doc(db, collectionName , await this.getDocId())
    
        updateDoc(ref, {
            nome: name.value.toLowerCase(),
            sobrenome: lastname.value.toLowerCase(),
            genero: gender.value,
    ...
    getDocId: async function(){ //13 - Antes de refatorar os elementos do Html tinham o id do doc firebase, remove isso e coloquei uma verifação para retornar o id
        var querySnapshot = await getDocs(usuarioCollectionRef); 
        
        let docId = null

        querySnapshot.forEach((doc)=> {
            
            if(this.docsDB[this.userId].data().cpf == doc.data().cpf  &&  this.docsDB[this.userId].data().nome == doc.data().nome){
                docId = doc.id
            }else{
                // console.log('erro ao verificar id do documento')
            }
        });
